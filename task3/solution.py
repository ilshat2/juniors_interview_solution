...
def appearance(intervals: dict[str, list[int]]) -> int:
    lesson = [(intervals['lesson'][i], intervals['lesson'][i + 1]) for i in range(0, len(intervals['lesson']), 2)]
    ...
    pupil_lesson = intersect_intervals(pupil, lesson)
    tutor_lesson = intersect_intervals(tutor, lesson)

    common_time = intersect_intervals(pupil_lesson, tutor_lesson)

    total_time = sum(end - start for start, end in common_time)
    return total_time
